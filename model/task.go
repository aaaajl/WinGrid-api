...
type TaskBillingContext struct {
	ModelPrice      float64                      `json:"model_price,omitempty"`       // 模型单价（per_duration 下为本次 costUSD）
	GroupRatio      float64                      `json:"group_ratio,omitempty"`       // 分组倍率
	ModelRatio      float64                      `json:"model_ratio,omitempty"`       // 模型倍率
	OtherRatios     map[string]float64           `json:"other_ratios,omitempty"`      // 附加倍率（时长、分辨率等）
	OriginModelName string                       `json:"origin_model_name,omitempty"` // 模型名称，必须为OriginModelName
	PerCallBilling  bool                         `json:"per_call_billing,omitempty"`  // 按次计费：跳过轮询阶段的差额结算
	BillingMode     string                       `json:"billing_mode,omitempty"`      // ratio / tiered_expr / per_duration
	Size            string                       `json:"size,omitempty"`              // per_duration 规范化分辨率
	Duration        int                          `json:"duration,omitempty"`          // per_duration 计费秒数
	BasePrice       float64                      `json:"base_price,omitempty"`        // per_duration USD/秒
	UsedFallback    bool                         `json:"used_fallback,omitempty"`     // per_duration 是否走 fallback
