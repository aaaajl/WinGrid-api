#!/usr/bin/env bash
# 从本地启停线上 LifeOS PostgreSQL。启动时会把可在生产机本机运行的同功能脚本
...
#
# 生产机本机:
#   /home/data/scripts/prod-longevityOS-pg.sh start|stop|status|check-backup
#
# 环境变量（均可选）:
...
run_onhost prod-longevityOS-pg.sh "${REMOTE_ARGS[@]}"
