#!/usr/bin/env bash
# 本地一键部署从节点到线上服务器：构建 linux/amd64 镜像 → 上传 → docker load → 重启
#
# 用法:
#   ./deploy-slave.sh                  # 构建、上传、load、重启（prod）
#   ./deploy-slave.sh --skip-build     # 跳过构建，使用已有 tar.gz
#   ./deploy-slave.sh --no-restart     # 只上传并 load，不重启
#   ./deploy-slave.sh --sync-scripts   # 强制同步 compose / start 脚本到远端
#   ./deploy-slave.sh --status         # 仅查看远程服务状态
#   ./deploy-slave.sh --no-cache       # 构建时不加缓存
#   ./deploy-slave.sh slave-1          # 指定 NODE_NAME（传给 start-slave.sh）
#
# 依赖本地:
#   ./buildProd.sh --save  →  ${IMAGE 安全名}-linux-amd64.tar.gz
# 远端目录须已有 .env（本脚本不会覆盖 .env；MASTER_HOST 等须已配置）
set -euo pipefail
...
REMOTE_DIR="${REMOTE_DIR:-/home/apps/longevity-new-api/slave}"
...
Options:
  ...
  --sync-scripts   强制用本地 compose / start-slave.sh 覆盖远端
...
Environment overrides:
  ...
  REMOTE_USER      默认 root
  REMOTE_DIR       默认 /home/apps/longevity-new-api/slave
  SSH_KEY          默认 \${REPO_ROOT}/localFiles/Longevity.pem
  SSH_PORT         默认 22
  IMAGE            默认 wingrid-api:local
  ENV_MODE         重启时传给 start-slave.sh，默认 prod
  ARCHIVE          本地镜像包路径（默认按 IMAGE 推导）
  NODE_NAME        从节点名；也可作位置参数传入

Examples:
  $0
  $0 --skip-build
  $0 new-api-slave-1
  REMOTE_DIR=/home/apps/longevity-new-api/slave-2 $0 --sync-scripts
EOF
}
...
sync_remote_scripts() {
  ...
  if [[ "${SYNC_SCRIPTS}" == "true" ]]; then
    echo "==> Force syncing slave scripts (--sync-scripts)"
  else
    echo "==> Remote scripts missing; uploading compose / start-slave.sh"
  fi

  for f in "${files[@]}"; do
    scp_to "${SCRIPT_DIR}/${f}" "${REMOTE_USER}@${REMOTE_HOST}:${REMOTE_DIR}/${f}"
  done
  remote "chmod +x '${REMOTE_DIR}/start-slave.sh'"
}
...
restart_remote() {
  ...
  echo "==> Restarting remote slave (ENV_MODE=${ENV_MODE}${NODE_NAME:+ NODE_NAME=${NODE_NAME}}) ..."
  remote bash -s <<EOF
set -euo pipefail
cd '${REMOTE_DIR}'
if [[ ! -f .env ]]; then
  echo "ERROR: ${REMOTE_DIR}/.env not found on server"
  echo "Copy .env.example to .env and fill secrets / MASTER_HOST before first deploy."
  exit 1
fi
if [[ ! -f start-slave.sh ]]; then
  echo "ERROR: ${REMOTE_DIR}/start-slave.sh not found on server"
  exit 1
fi
chmod +x start-slave.sh
# 显式传 IMAGE，且 start-slave.sh 会优先采用调用方值（不被 .env 覆盖）
if [[ -n '${node_arg}' ]]; then
  IMAGE='${IMAGE}' ./start-slave.sh '${ENV_MODE}' --restart '${node_arg}'
else
  IMAGE='${IMAGE}' ./start-slave.sh '${ENV_MODE}' --restart
fi
docker ps --filter name=new-api-slave --format 'table {{.Names}}\t{{.Status}}\t{{.Image}}\t{{.Ports}}'
EOF
}
...
if [[ -f start-slave.sh ]]; then
  chmod +x start-slave.sh
  docker compose -f docker-compose.slave.yml --env-file .env ps 2>/dev/null || true
  docker ps --filter name=new-api-slave --format 'table {{.Names}}\t{{.Status}}\t{{.Image}}\t{{.Ports}}' || true
  curl -fsS --connect-timeout 2 --max-time 5 "http://127.0.0.1:\${HOST_PORT:-3888}/api/status" 2>/dev/null || true
  echo
else
  echo "start-slave.sh missing"
  docker ps --filter name=new-api-slave || true
fi
EOF
}
