...
func RelayTaskSubmit(c *gin.Context, info *relaycommon.RelayInfo) (*TaskSubmitResult, *dto.TaskError) {
	...
	perDuration := billing_setting.IsPerDurationBilling(modelName)

	// 5. 计费估算：让适配器根据用户请求提供 OtherRatios（时长、分辨率等）
	//    必须在 ModelPriceHelperPerCall 之后调用（它会重建 PriceData）。
	//    ResolveOriginTask 可能已在 remix 路径中预设了 OtherRatios，此处合并。
	//    per_duration 已按 basePrice×duration 一次算清，禁止再乘 seconds/size。
	if !perDuration && info.TieredBillingSnapshot == nil {
	...
	_, adjustBillingOnComplete := adaptor.(channel.CompletionBillingAdaptor)
	return &TaskSubmitResult{
		...
		Platform:                platform,
		Quota:                   finalQuota,
		AdjustBillingOnComplete: adjustBillingOnComplete,
