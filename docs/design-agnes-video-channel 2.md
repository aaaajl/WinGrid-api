...
> 状态：M1–M4 已落地（M3 切流见 `docs/migrate-agnes-video-m3.sql`；现网渠道 `#30 Agnes Video CN` type=60）  
> 日期：2026-08-04  
...
>
> 实现摘要：`ChannelTypeAgnesVideo=60`、`relay/channel/task/agensvideo`、通用 `UpstreamVideoID` 提取与轮询透传；VideoProxy 走 default CDN 分支；存量 OpenAI+Sora 路径不变。
