...
func InjectPeakOffPeakBillingInfo(other *model.LogOther, relayInfo *relaycommon.RelayInfo, result *peakoffpeak.Result) {
	if relayInfo == nil || other == nil {
		return
	}
	...
	other.SetPublic("billing_mode", billing_setting.BillingModePeakOffPeak)
	if snap.EvalUnix != 0 {
		other.SetPublic("billing_eval_at", time.Unix(snap.EvalUnix, 0).UTC().Format(time.RFC3339))
	}
	...
	if period != "" {
		other.SetPublic("matched_tier", period)
	}
	prices := peakoffpeak.PricesForPeriod(snap.Config, period)
	// Absolute $/1M prices used for this request (matched period).
	other.SetPublic("peak_offpeak_input_price", prices.CacheMiss)
	other.SetPublic("peak_offpeak_output_price", prices.Completion)
	other.SetPublic("peak_offpeak_cache_hit_price", prices.CacheHit)
	other.SetPublic("peak_offpeak_pricing", map[string]any{
